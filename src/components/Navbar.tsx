...
export default function Navbar() {
  ...
  const navItems = ["Home", "Workexp", "Projects", "Blog", "About"];
          ...
          {navItems.map((item) => (
            ...
            >
              <Link
                ...
                className="text-lg italic text-blue-500 hover:text-blue-400 hover:font-bold transition-all no-underline duration-300 ease-in-out"
              >
                {item}
              </Link>
            </motion.div>
        ...
        >
          {navItems.map((item) => (
            <Link
              href={`/${item.toLowerCase()}`}
              key={item}
              className="block text-lg italic text-blue-500 hover:text-blue-400 hover:font-bold transition-all no-underline duration-300 ease-in-out"
            >
              {item}
            </Link>
          ))}
